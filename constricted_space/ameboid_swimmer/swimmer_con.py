...
#print(my_restored_policy1.model)
...
#         self.betamin = -(1.0*math.pi)/6.0
        self.betamax = 6*(math.pi)/18
        self.betamin = -6*(math.pi)/18
...
#                 print(staten[3:]-state_p)
    ...
    #         P_list=np.array(pressure_all)
                ...
                con_pre=self.con
                con=self.get_concentration(XY_positions)            
                con=np.squeeze(con)
                self.Clist_n=con.copy()
                #con_now=self.Clist_n[self.order]-np.mean(con)
                con_now=np.sum(con)
                
                reward+=(con_now-con_pre).item()*1000/N-np.max(abs(P_list))*0.0
                #reward+=con_now
                self.con=con_now
                #print(Xfirst)                
                #self.Xn+=Xn
                ...
                if loop_sign is False:
                    ...
                    for i in range(N):
                        if Ypositions[i]<0.01 or Ypositions[i]>1.99:
                ...
                #print(self.breakwall)
                
                if loop_sign is False and self.breakwall is False:
                    for i in range(N):
                        for j in range(51):
                            if 0.75*math.cos(math.pi*(Xpositions[i]+j/50*(Xpositions[(i+1)%N]-Xpositions[i])))+(Ypositions[i]+j/50*(Ypositions[(i+1)%N]-Ypositions[i]))-1.25>-0.01\
                ...
                if loop_sign is False and self.breakwall is False and self.breaki is False:
                    for i in range(N):
                        for j in range(N):
                            if i!=j and (i+1)%N!=j%N and (j+1)%N!=i%N:
                                if math.sqrt((Xpositions[i]-Xpositions[j])**2+(Ypositions[i]-Ypositions[j])**2)<0.6*wl/N:
                                    #print('touch the constriction')                
...
#                 self.breakp=False
#                 
#                 if loop_sign is False and self.breakwall is False and self.breaki is False and self.break_points is False :
#                     for i in range(N):  
#                         if abs(P_list[i])>3.0:
#                             print('over')
#                             reward-=0.1
...
#                 break
        ...
        if     it%20==0 :
            ...
            np.savetxt('Xfirst.pt',self.Xfirst, delimiter=',')   

        if it%20==0 :
            print(self.state[:2],it,self.reward,self.order,self.Clist.argsort().argsort())
            print(self.state)
            #print(self.Clist)
        #print(self.reward)
...
#         else:
        ...
        maxindices=np.argpartition(Clist_copy, -3)[-3:]
        self.order=np.random.choice(maxindices)
        #self.order=(cmax_index+(np.random.randint(5)-3)+N)%N
...
#         self.C_records=np.zeros((N))
#         self.pressure_index=np.ones((N))
        #self.order=np.random.randint(N)
        return self.order
