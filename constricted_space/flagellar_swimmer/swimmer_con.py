...
#         add_reward=0        
        ...
        for _ in range(itere):
            ...
            if p1_done==False:                    
                ...
                staten ,Xn ,r,x_first_delta,Xpositions,Ypositions,pressure_all=RK(self.state_n,action_real.astype(np.double),self.Xfirst)
                for i in range(N-1):
                ...
                con_now=np.sum(con)               
                reward+=(con_now-con_pre).item()*100000
                self.con=con_now
                ...
                for i in range(N+1):
                    if Ypositions[i]<0.025 or Ypositions[i]>1.975:
                ...
                for i in range(N):
                    for j in range(51):
                        if 0.85*math.cos(math.pi*(Xpositions[i]+j/50*(Xpositions[(i+1)]-Xpositions[i])))+(Ypositions[i]+j/50*(Ypositions[(i+1)]-Ypositions[i]))-1.15>-0.01\
                           and abs(Xpositions[i]+j/50*(Xpositions[(i+1)]-Xpositions[i]))<1:
...
#                         break
                ...
                if traj2==[]:
                    ...
                    traj2=np.concatenate((traj2.reshape(-1,6),m.reshape(1,-1)),axis=0)
