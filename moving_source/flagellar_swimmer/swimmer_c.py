...
#        print(self.x_tar)
    ...
    def step(self,action):
        ...
        if self.it==0:
            ...
            self.Xfirst=np.loadtxt('Xfirst.pt', delimiter=',')
            con=1/np.sqrt((np.squeeze(self.XY_positions[:,0])-self.CcenterX)**2+(np.squeeze(self.XY_positions[:,1])-self.CcenterY)**2)
...
#         add_reward=0        
        ...
        for _ in range(itere):
            ...
            self.a=0.001/self.R
            for i_v in range(10):
                self.CcenterX+=-0.02*self.a*self.R*math.sin(self.a*(0.2*(self.itc-1)+i_v*0.02))
                self.CcenterY+=0.02*self.a*self.R*math.cos(self.a*(0.2*(self.itc-1)+i_v*0.02))                      
            reward=0            
...
#                         break            
            ...
            if traj2==[]:
                ...
                traj2=np.concatenate((traj2.reshape(-1,7),m.reshape(1,-1)),axis=0)
...
#         print(self.state[3:] )
        if self.it%10==0 :
            print(self.state[:2],self.it,self.reward,self.order,self.CcenterX,self.CcenterY)
            print(self.con)
