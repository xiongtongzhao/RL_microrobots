...
#     bg_flow_r=torch.norm(bg_flow_U, dim=1)
    ...
    bg_flow_U=Ut*torch.cat(((torch.cos(math.pi*Yposition[:,0]/(NL))*torch.sin(math.pi*Yposition[:,1]/(NL))).reshape(-1,1)\
                         ,-(torch.cos(math.pi*Yposition[:,1]/(NL))*torch.sin(math.pi*Yposition[:,0]/(NL))).reshape(-1,1)),dim=0)
